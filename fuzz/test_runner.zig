//! Zig 0.16.0's own test runner, vendored for one word.
//!
//! Upstream is `lib/zig/compiler/test_runner.zig` in the pinned
//! toolchain. This copy differs from it in exactly one place, marked
//! `// zssl:` below: the fuzzing error path called
//! `std.debug.writeStackTrace`, which takes a `*const debug.StackTrace`,
//! with the `*builtin.StackTrace` that `@errorReturnTrace()` returns.
//! The two are different types — one is an ordered slice with a skipped
//! count, the other a ring buffer with a write cursor — so it does not
//! compile. `std.debug.writeErrorReturnTrace` is the function for that
//! parameter, and does the ring arithmetic the other cannot.
//!
//! The path is only instantiated under `--fuzz`, because
//! `std.testing.fuzz` fixes its callback at `anyerror!void` and the
//! runner's `catch |err| switch (err)` is therefore always compiled.
//! That is why `zig build test` has always worked and the coverage-
//! guided search has never run: not a defect in this library, and not
//! one we could route around from outside the runner.
//!
//! Used by the `test-fuzz` step alone. Every other suite stays on the
//! stock runner, so if this copy rots against a new compiler the CI
//! gates are untouched and only the search stops.
//!
//! Zig is MIT-licensed; this file is Zig's, with that one change. To
//! re-sync on a toolchain bump: copy the upstream file over it, re-apply
//! this header and the marked line, and delete both if upstream has
//! fixed it — `grep -n writeStackTrace` on the new runner is the check.
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    p: {
                        if (@errorReturnTrace()) |trace| {
                            // zssl: was `writeStackTrace`, which takes the other StackTrace.
